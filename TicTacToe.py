...
class TicTacToe:
    ...
    def __init__(self, difficulty):
        ...
        self.starting_player = random.choice([1, 2]) # 1=player 2=ai
    ...
    def __str__(self):
        ...
        str_arr = self.game_arr.astype(str)
    ...
    def __evaluate__(self):
        for combination in self.WINNING_COMBINATIONS:
            # player already won
            if np.array_equal(self.game_arr[combination], self.player_arr):
                self.player_won = True
                break
            # opponent already won
            elif np.array_equal(self.game_arr[combination], self.opponent_arr):
                self.opponent_won = True
                break
        # no more turns possible
        if not len(self.game_arr[self.game_arr == 0]):
            self.tie = True
